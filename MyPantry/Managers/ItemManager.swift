...
struct ItemManager: ItemManagerType {
    ...
    func fetchItems(for pantryId: String) async throws -> [Item] {
        ...
        let query = CKQuery(recordType: ItemConverter.recordType, predicate: predicate)

        let (matchResults, _) = try await ckDB.records(matching: query)
        let records = matchResults.compactMap { try? $0.1.get() }

        return records.compactMap { ItemConverter.fromRecord($0) }
    }

    func addItem(_ item: Item, to pantryId: String) async throws -> Item {
        let newItem = Item(name: item.name, quantity: item.quantity,
                           ...
                           status: item.status)
        let record = ItemConverter.toRecord(newItem)
        let savedRecord = try await ckDB.save(record)
        guard let savedItem = ItemConverter.fromRecord(savedRecord) else {
    ...
    func updateItem(_ item: Item, in _: String) async throws -> Item {
        ...
        for (key, value) in ItemConverter.toRecord(item) {
            record[key] = value
        }

        // Update record in cloud kit and verify
        let updatedRecord = try await ckDB.save(record)
        guard let updatedItem = ItemConverter.fromRecord(updatedRecord) else {
